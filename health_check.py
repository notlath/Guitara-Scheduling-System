...
def test_notification_endpoint():
    ...
    try:
        ...
        response = requests.get(f"{base_url}/api/scheduling/notifications/", headers=headers, timeout=5)
...
def check_database():
    ...
    try:
        ...
        for role in ['operator', 'therapist', 'driver']:
            ...
            print(f"📊 {role.title()}s: {users.count()} users, {notifications.count()} notifications")
