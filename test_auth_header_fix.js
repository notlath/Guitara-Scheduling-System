...
testCases.forEach(({ token, description }) => {
  ...
  console.log(
    `  ${description}: ${isValid ? "✅ Valid" : "❌ Invalid"} - ${
      shouldAddHeader ? "ADD header" : "NO header"
    }`
  );
});
...
testCases.forEach(({ token, description }) => {
  ...
  console.log(
    `  ${description}: ${hasAuthHeader ? "✅" : "❌"} Header: ${headerValue}`
  );
});
...
console.log(
  "✅ Login requests will now be sent without Authorization header when no valid token exists."
);
