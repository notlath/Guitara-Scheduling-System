...
def test_availability_manager_disabled_staff():
    ...
    response = requests.get(f"{API_BASE_URL}/scheduling/staff-members/", headers=headers)
    if response.status_code != 200:
    ...
    print(f"✅ Found therapist: {test_therapist['first_name']} {test_therapist['last_name']} (ID: {test_therapist['id']})")
    print(f"   Current status: {'Active' if test_therapist['is_active'] else 'Disabled'}")
    ...
    if test_therapist['is_active']:
        ...
        if response.status_code != 200:
            print(f"❌ Failed to disable therapist: {response.status_code} - {response.text}")
            return False
    ...
    if response.status_code == 200:
        ...
        print(f"✅ Can view availability for disabled staff (found {len(availability)} entries)")
    else:
    ...
    if response.status_code != 201:
        print(f"✅ Adding availability for disabled staff correctly prevented: {response.status_code}")
        if response.status_code == 400:
            error_data = response.json()
            print(f"   Error message: {error_data}")
    else:
        print(f"❌ Adding availability for disabled staff was allowed (should be prevented)")
        return False
    ...
    if response.status_code == 200:
        ...
        print(f"   New status: {'Active' if result['user']['is_active'] else 'Disabled'}")
    else:
        print(f"❌ Failed to toggle account status: {response.status_code} - {response.text}")
        return False
    ...
    if response.status_code == 201:
        ...
        print(f"❌ Failed to add availability for enabled staff: {response.status_code}")
        return False
