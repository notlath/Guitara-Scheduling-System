...
def debug_staff_data():
    ...
    for i, staff in enumerate(staff_members, 1):
        ...
        print(
            f"  is_active: {staff.get('is_active')} (type: {type(staff.get('is_active'))})"
        )
    ...
    if staff_members:
        ...
        response = requests.patch(
            f"{API_BASE_URL}/toggle-account-status/{test_staff['id']}/", headers=headers
        )
        ...
        if response.status_code == 200:
            ...
            print(
                f"New is_active value: {updated_user['is_active']} (type: {type(updated_user['is_active'])})"
            )
        else:
