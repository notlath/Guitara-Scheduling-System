...
def check_database_staff():
    ...
    try:
        ...
        for row in results:
            ...
            print(
                f"ID: {id:2} | {first_name:10} {last_name:10} | {role:10} | {status:8} | is_active: {is_active}"
            )
        ...
        if disabled_count > 0:
            ...
            print(
                f"UPDATE core_customuser SET is_active = 1 WHERE role IN ('therapist', 'driver') AND is_active = 0;"
            )
