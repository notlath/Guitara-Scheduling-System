...
class SchedulingViewPersistenceValidator {
  ...
  getCurrentVisibleContent() {
    ...
    if (contentArea.querySelector(".calendar-container, .calendar"))
      return "calendar";
    ...
    if (contentArea.querySelector(".availability-manager"))
      return "availability";
  ...
  validateCurrentState() {
    ...
    const result = {
      ...
      tests: {
        urlParameterPresent:
          urlView !== "calendar" || window.location.search === "",
        correctButtonActive:
          activeButton &&
          activeButton.textContent.trim() === this.viewMappings[urlView],
        correctContentVisible: visibleContent === urlView,
      },
    };

    result.allTestsPassed = Object.values(result.tests).every(
      (test) => test === true
    );
  ...
  displayFailures(validation) {
    ...
    if (!validation.tests.correctButtonActive) {
      this.log(
        `  - Wrong button active: expected "${validation.expectedButtonText}", got "${validation.actualButtonText}"`,
        "error"
      );
    }
    if (!validation.tests.correctContentVisible) {
      this.log(
        `  - Wrong content visible: expected "${validation.expectedView}", got "${validation.visibleContent}"`,
        "error"
      );
    }
  }
  ...
  async runFullValidation() {
    this.log(
      "🚀 Starting Full SchedulingDashboard View Persistence Validation",
      "info"
    );
    this.log(
      "================================================================",
    ...
    if (currentStateResult.allTestsPassed && allSwitchingPassed) {
      this.log(
        "✅ ALL TESTS PASSED - View persistence is working correctly!",
        "success"
      );
    } else {
      this.log(
        "❌ SOME TESTS FAILED - View persistence needs attention",
        "error"
      );
    }
