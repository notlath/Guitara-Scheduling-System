...
def test_real_time_sync():
    ...
    print(
        f"✅ Found therapist: {therapist_user['first_name']} {therapist_user['last_name']} (ID: {therapist_id})"
    )
    ...
    if response.status_code != 201:
        print(
            f"❌ Failed to create availability: {response.status_code} - {response.text}"
        )
        return False
    ...
    if response.status_code == 200:
        ...
        print(
            f"❌ Failed to update availability: {response.status_code} - {response.text}"
        )
    ...
    if response.status_code == 204:
        ...
        print(
            f"❌ Failed to delete availability: {response.status_code} - {response.text}"
        )
    ...
    print(
        "- OperatorDashboard should refresh automatically when therapist adds availability"
    )
