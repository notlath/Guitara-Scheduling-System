...
def test_availability_creation():
    ...
    try:
        ...
        print(
            "✅ Test 1 PASSED: Successfully created availability for active therapist"
        )
    ...
    print(
        f"   - Disabled therapist: {disabled_therapist.first_name} {disabled_therapist.last_name}"
    )
    print(
        f"   - Account status: {'Active' if disabled_therapist.is_active else 'DISABLED'}"
    )
    ...
    if not disabled_therapist.is_active:
        print(
            "❌ Test 2 EXPECTED: Account is disabled - availability creation should be blocked"
        )
        print(
            f"   Error message: Cannot create availability for {disabled_therapist.first_name} {disabled_therapist.last_name}."
        )
        print(
    ...
    for user in all_users:
        ...
        print(
            f"   {user.first_name} {user.last_name} ({user.role}): {status} - {availability_count} availabilities"
        )
