...
try:
    ...
    def test_staff_data():
        ...
        for staff in staff_members:
            ...
            print(
                f"   Raw is_active: {staff.is_active} (type: {type(staff.is_active)})"
            )
            ...
            print(
                f"   After JSON round-trip - is_active: {parsed_data.get('is_active')} (type: {type(parsed_data.get('is_active'))})"
            )
        ...
        print(
            f"Boolean True -> Serialized: {serializer.data.get('is_active')} (type: {type(serializer.data.get('is_active'))})"
        )
        ...
        serializer = UserSerializer(test_user)
        print(
            f"Boolean False -> Serialized: {serializer.data.get('is_active')} (type: {type(serializer.data.get('is_active'))})"
        )
