...
def check_operator_dashboard_fixes():
    ...
    dashboard_path = os.path.join(os.getcwd(), "royal-care-frontend", "src", "components", "OperatorDashboard.jsx")
    ...
    has_useSelector_staffMembers = "const { appointments, notifications, staffMembers, loading, error } = useSelector(" in content
    ...
    if all([has_memoized_drivers, has_useSelector_staffMembers, has_callback_function, has_memoized_pickup]):
...
def check_scheduling_slice():
    ...
    slice_path = os.path.join(os.getcwd(), "royal-care-frontend", "src", "features", "scheduling", "schedulingSlice.js")
...
def main():
    ...
    print("\n" + "=" * 50)
    if all_tests_passed:
        print("🎉 All tests passed! OperatorDashboard.jsx should now work without runtime errors.")
        print("\n📋 Next steps:")
