...
const NotificationDebugTool = () => {
  ...
  useEffect(() => {
    ...
    setDebugInfo(debug);
  }, [notifications, unreadCount, notificationStatus, error, user, authToken, isAuthenticated]);
  ...
  return (
    <div style={{
      ...
      <h3 style={{ margin: '0 0 10px 0', color: '#333' }}>🔍 Notification Debug Tool</h3>
      ...
      {debugInfo && (
        <div style={{ marginBottom: '15px' }}>
          <h4 style={{ margin: '0 0 5px 0', color: '#007bff' }}>Redux State:</h4>
          <pre style={{ backgroundColor: '#f8f9fa', padding: '8px', borderRadius: '4px', margin: 0 }}>
            {JSON.stringify(debugInfo.redux, null, 2)}
          </pre>
          
          <h4 style={{ margin: '10px 0 5px 0', color: '#007bff' }}>Auth State:</h4>
          <pre style={{ backgroundColor: '#f8f9fa', padding: '8px', borderRadius: '4px', margin: 0 }}>
            {JSON.stringify(debugInfo.auth, null, 2)}
          </pre>
          
          <h4 style={{ margin: '10px 0 5px 0', color: '#007bff' }}>Browser Storage:</h4>
          <pre style={{ backgroundColor: '#f8f9fa', padding: '8px', borderRadius: '4px', margin: 0 }}>
      ...
      {manualFetch && (
        <div>
          <h4 style={{ margin: '10px 0 5px 0', color: '#28a745' }}>Manual Fetch Result:</h4>
          <pre style={{ 
