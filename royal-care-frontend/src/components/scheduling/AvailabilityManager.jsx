...
const isStaffActive = (staff) => {
  ...
  console.warn(`Warning: Unexpected is_active value for ${staff.first_name} ${staff.last_name}, defaulting to true:`, isActive, typeof isActive);
  return true;
};
...
const AvailabilityManager = () => {
  ...
  const [newAvailabilityForm, setNewAvailabilityForm] = useState({
    ...
    isAvailable: true,  });  // Fetch staff members and availabilities on component mount
  useEffect(() => {
  ...
  }, [selectedDate]);  const handleStaffChange = (e) => {
    const staffId = e.target.value;
    setSelectedStaff(staffId);
      // Find and store the selected staff data
    if (staffId) {
      const staffData = staffMembers.find(staff => staff.id.toString() === staffId);
      if (staffData) {
        ...
        console.log("🧪 Debug: is_active evaluation:", isStaffActive(staffData));
      }
    } else {
  ...
  const handleToggleAvailability = (availability) => {
    ...
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/toggle-account-status/${selectedStaffData.id}/`, {
        method: 'PATCH',
        headers: {
      ...
      {error && (
        ...
        {user.role === "operator" && (
          <div className="filter-group">
            ...
            <div className="debug-section" style={{marginTop: '10px', padding: '10px', backgroundColor: '#f5f5f5', border: '1px solid #ddd', borderRadius: '4px'}}>
              <details>
                <summary style={{cursor: 'pointer', fontWeight: 'bold'}}>🔍 Debug Staff Status</summary>
                <div style={{marginTop: '10px'}}>
                  <button 
                    onClick={() => dispatch(fetchStaffMembers())}
                    style={{marginBottom: '10px', padding: '5px 10px', backgroundColor: '#007cba', color: 'white', border: 'none', borderRadius: '3px', cursor: 'pointer'}}
                  >
                    🔄 Refresh Staff Data
                  </button>
                  <div style={{fontSize: '12px', fontFamily: 'monospace'}}>
                    <strong>Staff Members ({filteredStaffMembers.length} found):</strong>
                    {filteredStaffMembers.map((staff) => (
                      <div key={staff.id} style={{margin: '5px 0', padding: '5px', backgroundColor: 'white', border: '1px solid #ccc'}}>
                        <div><strong>{staff.first_name} {staff.last_name}</strong> (ID: {staff.id})</div>
                        <div>Role: {staff.role}</div>
                        <div>Raw is_active: {String(staff.is_active)} (type: {typeof staff.is_active})</div>
                        <div>Evaluated: {isStaffActive(staff) ? 'ACTIVE' : 'DISABLED'}</div>
                      </div>
                    ))}
                  </div>
                </div>
        ...
        {selectedStaffData && !isStaffActive(selectedStaffData) && (
          <div className="disabled-staff-warning">
            ...
            <div className="warning-content">
              ...
              {selectedStaffData.first_name} {selectedStaffData.last_name}'s account is currently disabled. 
              You can view their current availability below, but cannot add new availability. 
              Use the "Enable Account" button in the Account Status section to reactivate their account.
            </div>
          </div>
        )}
        ...
        {user.role === "operator" && selectedStaffData && (
          <div className="account-status-section">
            <h4>Account Status for {selectedStaffData.first_name} {selectedStaffData.last_name}</h4>            <div className="account-status-info">
              <div className="status-indicator">
                <span className={`status-badge ${isStaffActive(selectedStaffData) ? 'active' : 'inactive'}`}>
              ...
              <button 
                className={`toggle-account-button ${isStaffActive(selectedStaffData) ? 'disable-btn' : 'enable-btn'}`}
                onClick={handleToggleAccountStatus}
                title={isStaffActive(selectedStaffData) ? 'Disable this account' : 'Enable this account'}
              >
                {isStaffActive(selectedStaffData) ? 'Disable Account' : 'Enable Account'}
              </button>
            </div>
            
            {!isStaffActive(selectedStaffData) && (
              <div className="account-disabled-note">
                <strong>Note:</strong> This account is disabled. The staff member cannot log in or receive new appointments. 
                Click "Enable Account" to reactivate.
              </div>
            )}
          </div>
        )}
