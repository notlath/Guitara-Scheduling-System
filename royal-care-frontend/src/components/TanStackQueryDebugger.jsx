...
const TanStackQueryDebugger = () => {
  ...
  const debugQueryCache = () => {
    ...
    const appointmentsCache = queryClient.getQueryData(['appointments', 'list']);
  ...
  return (
    <div style={{ 
      ...
      <h4 style={{ margin: '0 0 10px 0', color: '#007bff' }}>TanStack Query Debugger</h4>
      
      <div style={{ marginBottom: '8px', fontSize: '12px' }}>
        <strong>Data Counts:</strong><br/>
        Appointments: {dashboardData.appointments?.length || 0}<br/>
        Today: {dashboardData.todayAppointments?.length || 0}<br/>
        Notifications: {dashboardData.notifications?.length || 0}
      </div>
      
      <div style={{ marginBottom: '8px', fontSize: '12px' }}>
        <strong>States:</strong><br/>
        Loading: {dashboardData.loading ? 'Yes' : 'No'}<br/>
        Error: {dashboardData.error ? 'Yes' : 'No'}<br/>
