...
export const useBulkOperations = () => {
  ...
  const executeBulkOperation = useCallback(async ({
    ...
    try {
      ...
      for (let i = 0; i < items.length; i += batchSize) {
        ...
        const batchPromises = batch.map(async (item) => {
          try {
            ...
            const errorInfo = { item, error: error.message || 'Unknown error' };
      ...
      setBulkErrors(prev => ({
        ...prev,
        [progressKey]: [...(prev[progressKey] || []), { error: error.message }]
      }));
      throw error;
    } finally {
  ...
  const bulkApproveAppointments = useCallback(async (appointmentIds) => {
  ...
  // Bulk assign drivers
  const bulkAssignDrivers = useCallback(async (assignments) => {
    return executeBulkOperation({
      items: assignments,
      operation: async ({ appointmentId, driverId }) => {
        const response = await fetch(`/api/appointments/${appointmentId}/assign_driver/`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ driver_id: driverId })
        });
        if (!response.ok) throw new Error(`Driver assignment failed: ${response.statusText}`);
        return response.json();
      },
  ...
  // Bulk mark as paid
  const bulkMarkAsPaid = useCallback(async (appointmentIds, paymentData = {}) => {
    return executeBulkOperation({
      items: appointmentIds,
      operation: async (appointmentId) => {
        const response = await fetch(`/api/appointments/${appointmentId}/mark_paid/`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(paymentData)
        });
        if (!response.ok) throw new Error(`Payment marking failed: ${response.statusText}`);
        return response.json();
      },
  ...
  const bulkCancelAppointments = useCallback(async (appointmentIds, reason = '') => {
    return executeBulkOperation({
      items: appointmentIds,
      operation: async (appointmentId) => {
  ...
  const getProgress = useCallback((progressKey) => {
    return bulkProgress[progressKey] || null;
  }, [bulkProgress]);

  // Get errors for a specific operation
  const getErrors = useCallback((progressKey) => {
