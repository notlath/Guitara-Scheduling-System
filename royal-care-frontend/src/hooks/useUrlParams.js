 ...
 */
export const useUrlParams = (validViewValues = [], validFilterValues = [], defaultView = "today") => {
  ...
  const currentView = validViewValues.length > 0 
    ? validateUrlParam(rawView, validViewValues, defaultView)
    : (rawView || defaultView);
  
  const currentFilter = validFilterValues.length > 0
  ...
  useEffect(() => {
    const warnings = [];
    if (rawView && validViewValues.length > 0 && !validViewValues.includes(rawView)) {
      warnings.push(`Invalid view "${rawView}" reset to "${defaultView}"`);
    }
    if (rawFilter && validFilterValues.length > 0 && !validFilterValues.includes(rawFilter)) {
      warnings.push(`Invalid filter "${rawFilter}" reset to "all"`);
    }
    setValidationWarnings(warnings);
  }, [rawView, rawFilter, validViewValues, validFilterValues, defaultView]);
