...
export const useAvailableTherapists = (
  ...
  // Generate end time if not provided (matches your existing logic)
  const calculatedEndTime =
    endTime ||
    (() => {
      if (!startTime || !serviceId) return null;
      // This would need your service duration logic
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      if (calculatedEndTime) {
        params.end_time = calculatedEndTime;
      }

      const result = await dispatch(fetchAvailableTherapists(params)).unwrap();
      return result;
    },
    enabled: !!(date && startTime && serviceId && isValidToken()),
...
export const useAvailableDrivers = (date, startTime, endTime = null) => {
  ...
  return useQuery({
    ...
    enabled: !!(date && startTime && isValidToken()),
