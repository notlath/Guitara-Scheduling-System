...
export const useWebSocketCacheSync = (webSocketService) => {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      webSocketService.removeEventListener('therapist_response', handleUserResponse);
      webSocketService.removeEventListener('driver_response', handleUserResponse);
    };
...
export const useCacheInvalidation = () => {
  ...
  const invalidateAppointments = async (options = {}) => {
    const { invalidateAppointmentCaches } = await import('../utils/cacheInvalidation');
