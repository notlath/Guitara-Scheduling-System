...
def login_and_get_token():
    ...
    try:
        with PerformanceTimer("User Login"):
            response = requests.post(f"{BACKEND_URL}/api/authentication/login/", json=login_data)
...
async def test_real_time_update(token):
    ...
    try:
        ...
        async with websockets.connect(uri) as websocket:
            ...
            print("⏳ Waiting for real-time updates (create an appointment in another tab)...")
            ...
            try:
                ...
                while time.time() - start_time < 30:
                    ...
                    if data.get("type") in ["appointment_created", "appointment_updated"]:
