...
class AppointmentViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        if materials_raw:
            logger.info(
                f"Materials length: {len(materials_raw) if hasattr(materials_raw, '__len__') else 'N/A'}"
            )
        ...
        if hasattr(serializer, "initial_data"):
            ...
            logger.info(
                f"Materials in serializer initial_data: {serializer.initial_data.get('materials', 'NOT_FOUND')}"
            )

        # Check serializer validated data
        if hasattr(serializer, "validated_data"):
            logger.info(
                f"Serializer validated_data keys: {list(serializer.validated_data.keys())}"
            )
        ...
        if materials_raw:
            logger.info(
                f"🔧 CREATING AppointmentMaterial records for {len(materials_raw)} materials"
            )
            ...
            for i, material_data in enumerate(materials_raw):
                try:
                    ...
                    if not isinstance(material_data, dict):
                        logger.error(
                            f"Material {i+1} is not a dict: {type(material_data)} - {material_data}"
                        )
                        continue
                    ...
                    if item_id:
                        try:
                            ...
                            logger.info(
                                f"✅ Found inventory item by ID {item_id}: {inventory_item.name}"
                            )
                        ...
                        try:
                            ...
                            logger.info(
                                f"✅ Found inventory item by name '{item_name}': {inventory_item.name}"
                            )
                        except InventoryItem.DoesNotExist:
                            logger.error(
                                f"❌ Inventory item with name '{item_name}' not found"
                            )

                    if not inventory_item:
                        logger.error(
                            f"❌ Could not find inventory item for material: {material_data}"
                        )
                        continue
                    ...
                    logger.info(
                        f"🔧 Creating AppointmentMaterial: {inventory_item.name} x{quantity_used}"
                    )
                    ...
                    logger.info(
                        f"✅ Created AppointmentMaterial: {inventory_item.name} x{quantity_used}"
                    )

                    # Check current inventory before moving
                    logger.info(
                        f"🔍 Before moving: {inventory_item.name} - stock: {inventory_item.current_stock}, in_use: {inventory_item.in_use}"
                    )
                    ...
                    if inventory_item.move_to_in_use(quantity_used):
                        ...
                        logger.info(
                            f"✅ Moved {quantity_used} {inventory_item.name} from stock to in_use"
                        )
                        logger.info(
                            f"🔍 After moving: {inventory_item.name} - stock: {inventory_item.current_stock}, in_use: {inventory_item.in_use}"
                        )
                        ...
                        try:
                            ...
                            logger.info(
                                f"✅ Created usage log for {inventory_item.name}"
                            )
                        except ImportError:
                        ...
                        logger.error(
                            f"❌ Failed to move {inventory_item.name} to in_use - insufficient stock (available: {inventory_item.current_stock}, needed: {quantity_used})"
                        )
                        # Should we rollback the AppointmentMaterial creation?
                        appointment_material.delete()
                        logger.warning(
                            f"⚠️ Deleted AppointmentMaterial due to insufficient stock"
                        )

                except (ValueError, TypeError) as data_error:
                    logger.error(
                        f"❌ Data validation error for material {i+1}: {data_error}"
                    )
                except Exception as e:
                    logger.error(
                        f"❌ Unexpected error creating AppointmentMaterial for material {i+1}: {e}"
                    )
                    import traceback
            ...
            logger.info(
                f"🔧 Finished processing materials for appointment {appointment.id}"
            )
    ...
    def complete(self, request, pk=None):
        ...
        print(
            f"🔍 complete: Request user: {request.user} (role: {getattr(request.user, 'role', 'unknown')})"
        )
        ...
        print(
            f"🔍 Materials array length: {len(materials_array) if materials_array else 0}"
        )

        # Process the new materials format
        if materials_array and len(materials_array) > 0:
            print(
                f"🚨 PROCESSING MATERIALS ARRAY! Found {len(materials_array)} materials"
            )
                ...
                for i, material_data in enumerate(materials_array):
                    ...
                    try:
                        ...
                        appointment_material, created = (
                            AppointmentMaterial.objects.get_or_create(
                                appointment=appointment,
                                inventory_item=inventory_item,
                                defaults={
                                    "quantity_used": quantity_used,
                        ...
                        if created:
                            print(
                                f"✅ Created AppointmentMaterial: {appointment_material}"
                            )
                        else:
                            print(
                                f"✅ Found existing AppointmentMaterial: {appointment_material}"
                            )

                        # Now update inventory based on status
                        print(
                            f"🔍 Current inventory state: stock={inventory_item.current_stock}, in_use={inventory_item.in_use}, empty={inventory_item.empty}"
                        )

                        if status == "empty":
                            print(
                                f"🔥 MOVING {quantity_used} units of {inventory_item.name} TO EMPTY!"
                            )
                            ...
                            if inventory_item.current_stock >= quantity_used:
                                ...
                                inventory_item.save()
                                print(
                                    f"✅ MOVED TO EMPTY! New values: stock={inventory_item.current_stock}, empty={inventory_item.empty}"
                                )
                            else:
                            ...
                            print(
                                f"🔥 MOVING {quantity_used} units of {inventory_item.name} TO IN_USE!"
                            )
                            ...
                            if inventory_item.current_stock >= quantity_used:
                                ...
                                inventory_item.save()
                                print(
                                    f"✅ MOVED TO IN_USE! New values: stock={inventory_item.current_stock}, in_use={inventory_item.in_use}"
                                )
                            else:
                                print(f"❌ Not enough stock to move to in_use!")

                        print(
                            f"🔍 Final inventory state: stock={inventory_item.current_stock}, in_use={inventory_item.in_use}, empty={inventory_item.empty}"
                        )
        ...
        print(
            f"  - materials_checked: {materials_checked} (type: {type(materials_checked)})"
        )
        print(
            f"  - materials_are_empty: {materials_are_empty} (type: {type(materials_are_empty)})"
        )
        ...
        if materials_checked:
            print(
                f"✅ MATERIALS CHECK CONDITION MET! Processing materials for appointment {appointment.id}"
            )
            ...
            try:
                ...
                if len(appointment_materials) == 0:
                    ...
                    print(
                        f"   This appointment was created before the fix was applied."
                    )
                    print(
                        f"   Skipping inventory updates - please recreate the appointment to get proper material tracking."
                    )
                    return Response(
                        {
                ...
                for appointment_material in appointment_materials:
                    ...
                    if materials_are_empty:
                        ...
                        print(
                            f"   - Moving {quantity_used} units from in_use to empty..."
                        )
                        ...
                        print(
                            f"   - New values: in_use={inventory_item.in_use}, empty={inventory_item.empty}"
                        )
                    else:
                        # Move materials from "in_use" back to "in_stock"
                        print(
                            f"   - Returning {quantity_used} units from in_use to stock..."
                        )
                        ...
                        print(
                            f"   - New values: in_use={inventory_item.in_use}, current_stock={inventory_item.current_stock}"
                        )

                print(
                    f"✅ All materials processed successfully for appointment {appointment.id}"
                )
        ...
        print(
            f"❌ WARNING: /complete/ endpoint called for appointment {appointment.id}"
        )
    ...
    def mark_completed(self, request, pk=None):
        ...
        if appointment.status != "payment_verified":
            print(
                f"❌ ERROR: Status changed from payment_verified to {appointment.status}!"
            )
        else:
        ...
        try:
            ...
            for appointment_material in appointment.appointment_materials.all():
                ...
                if materials_are_empty:
                    ...
                    print(
                        f"✅ Moved {quantity_used} {inventory_item.name} from in_use to empty"
                    )
                else:
                    # Move materials from "in_use" back to "in_stock"
                    success = inventory_item.return_to_stock(quantity_used)
                    ...
                    print(
                        f"✅ Returned {quantity_used} {inventory_item.name} from in_use to stock"
                    )
            ...
            # Create notifications
            material_status_message = (
                "marked as empty" if materials_are_empty else "returned to stock"
            )
            self._create_notifications(
                appointment,
                "materials_checked",
