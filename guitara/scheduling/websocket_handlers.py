    ...
    def broadcast_appointment_created(appointment):
        ...
        try:
            event_data = {
                ...
                "appointment": AppointmentWebSocketHandler._serialize_appointment(appointment),
                "timestamp": timezone.now().isoformat(),
                "created_by": appointment.created_by.id if appointment.created_by else None,
            }
            
            # Broadcast to all appointment listeners
            async_to_sync(channel_layer.group_send)("appointments", {
            ...
            if appointment.therapists.exists():
                for therapist in appointment.therapists.all():
                    async_to_sync(channel_layer.group_send)(f"user_{therapist.id}", {
                        "type": "send_appointment_update",
                        "data": {
                            **event_data,
            ...
            if appointment.driver:
                async_to_sync(channel_layer.group_send)(f"user_{appointment.driver.id}", {
                    "type": "send_appointment_update", 
                    "data": {
                        **event_data,
    ...
    def broadcast_appointment_updated(appointment, updated_fields=None):
        ...
        try:
            ...
            if "status" in (updated_fields or []):
                AppointmentWebSocketHandler._handle_status_change(appointment, event_data)
    ...
    def broadcast_therapist_response(appointment, therapist, accepted):
        ...
        try:
            ...
            for other_therapist in appointment.therapists.exclude(id=therapist.id):
                async_to_sync(channel_layer.group_send)(f"user_{other_therapist.id}", {
                    "type": "send_appointment_update",
                    "data": {
                        **event_data,
    ...
    def _handle_status_change(appointment, base_event_data):
        ...
        if status in status_messages:
            ...
            if appointment.client:
                async_to_sync(channel_layer.group_send)(f"user_{appointment.client.id}", {
                    "type": "send_appointment_update",
                    "data": {
                        **base_event_data,
    ...
    def _serialize_appointment(appointment):
        ...
        try:
            return {
                ...
                } if appointment.driver else None,
                ...
                ] if hasattr(appointment, 'services') else [],
                "total_amount": float(appointment.total_amount) if hasattr(appointment, 'total_amount') and appointment.total_amount else 0,
                "created_at": appointment.created_at.isoformat() if hasattr(appointment, 'created_at') and appointment.created_at else None,
                "updated_at": appointment.updated_at.isoformat() if hasattr(appointment, 'updated_at') and appointment.updated_at else None,
            }
        except Exception as e:
...
class NotificationWebSocketHandler:
    ...
    @staticmethod
    def broadcast_system_notification(notification_type, title, message, target_roles=None):
        ...
        try:
            ...
            if target_roles:
                for role in target_roles:
                    async_to_sync(channel_layer.group_send)(f"{role}s", {
                        "type": "send_notification",
                        "data": notification_data
                    })
            else:
