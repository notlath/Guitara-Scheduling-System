...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            self.stdout.write(
                f"✓ Found {user_notifications.count()} notifications for user"
            )
            ...
            if user_notifications.exists():
                for notif in user_notifications[:3]:
                    try:
                        ...
                        self.stdout.write(
                            f"✓ Serialized notification {notif.id} successfully"
                        )
                    except Exception as e:
                        self.stdout.write(
                            self.style.ERROR(
                                f"✗ Serialization failed for notification {notif.id}: {e}"
                            )
                        )
                        traceback.print_exc()
            else:
                ...
                try:
                    ...
                    data = serializer.data
                    self.stdout.write(
                        f"✓ Serialized test notification successfully: {data}"
                    )
                except Exception as e:
                    self.stdout.write(
                        self.style.ERROR(
                            f"✗ Test notification serialization failed: {e}"
                        )
                    )
                    traceback.print_exc()
            ...
            try:
                ...
                self.stdout.write(
                    f"✓ ViewSet get_queryset returned {queryset.count()} notifications"
                )
            except Exception as e:
                self.stdout.write(
                    self.style.ERROR(f"✗ ViewSet get_queryset failed: {e}")
                )
                traceback.print_exc()
            ...
            try:
                ...
                self.stdout.write(
                    f"✓ ViewSet list method returned status {response.status_code}"
                )
                if hasattr(response, "data"):
                    self.stdout.write(
                        f"✓ Response data length: {len(response.data) if isinstance(response.data, list) else 'Not a list'}"
                    )
            except Exception as e:
                self.stdout.write(
                    self.style.ERROR(f"✗ ViewSet list method failed: {e}")
                )
                traceback.print_exc()
