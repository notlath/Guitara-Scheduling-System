    ...
    def handle(self, *args, **options):
        ...
        for client_data in clients_data:
            if not Client.objects.filter(phone_number=client_data["phone_number"]).exists():
                ...
                self.stdout.write(
                    self.style.SUCCESS(f"✓ Created: {client_data['first_name']} {client_data['last_name']}")
                )
                created += 1
            else:
                self.stdout.write(f"- Already exists: {client_data['first_name']} {client_data['last_name']}")
        
        self.stdout.write(f"\nCreated {created} new clients")
        self.stdout.write(self.style.SUCCESS(f"Total clients now: {Client.objects.count()}"))
        
        # Display all clients
        self.stdout.write("\nAll clients in database:")
        for client in Client.objects.all():
            self.stdout.write(f"  - {client.first_name} {client.last_name} ({client.phone_number})")
