...
def get_user(token_key):
    ...
    try:
        ...
        print(f"[MIDDLEWARE] Token bytes type: {type(token_bytes)}, length: {len(token_bytes)}")
        
        user, auth_token = token_auth.authenticate_credentials(
            token_bytes
        )  # Authentication successful
        
        print(f"[MIDDLEWARE] ✅ Authentication successful for user: {user.id} ({user.username})")
        return user
...
class TokenAuthMiddleware:
    ...
    async def __call__(self, scope, receive, send):
        try:
            ...
            print(f"[MIDDLEWARE] Extracted token: {token_key[:20] if token_key else 'None'}...")
            ...
            if token_key:
                ...
                if scope["user"].is_authenticated:
                    ...
                    print(f"[MIDDLEWARE] User object: {scope['user']}, is_authenticated: {scope['user'].is_authenticated}")
            else:
