...
def test_appointment_workflow():
    ...
    try:
        ...
        print(f"✅ Found users: Operator({operator.username}), Therapist({therapist.username}), Driver({driver.username})")
        ...
        assert appointment.status == 'pending', f"Expected 'pending', got '{appointment.status}'"
