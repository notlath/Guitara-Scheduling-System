...
def test_confirmation_logic():
    ...
    print(
        f"📋 Appointment fields: {[f for f in appointment_fields if 'group' in f or 'confirmed' in f or 'requires_car' in f]}"
    )

    missing_appointment_fields = [
        f for f in required_appointment_fields if f not in appointment_fields
    ]
