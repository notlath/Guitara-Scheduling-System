...
def test_complete_workflow():
    ...
    if not all([operator, therapist, driver]):
        print(
            f"❌ Missing users - Operator: {bool(operator)}, Therapist: {bool(therapist)}, Driver: {bool(driver)}"
        )
        return False
    ...
    try:
        ...
        response = api_client.post(
            f"/api/scheduling/appointments/{appointment.id}/therapist_confirm/"
        )
        if response.status_code == 200:
            ...
            print(
                f"❌ Step 2 failed: {response.status_code} - {response.content.decode()}"
            )
            return False
        ...
        response = api_client.post(
            f"/api/scheduling/appointments/{appointment.id}/driver_confirm/"
        )
        if response.status_code == 200:
            appointment.refresh_from_db()
            ...
            print(
                f"❌ Step 3 failed: {response.status_code} - {response.content.decode()}"
            )
            return False
        ...
        response = api_client.post(
            f"/api/scheduling/appointments/{appointment.id}/start_appointment/"
        )
        if response.status_code == 200:
            appointment.refresh_from_db()
            print(
                f"✅ Step 4: Operator started appointment - status: {appointment.status}"
            )
        else:
            print(
                f"❌ Step 4 failed: {response.status_code} - {response.content.decode()}"
            )
            return False
        ...
        response = api_client.post(
            f"/api/scheduling/appointments/{appointment.id}/start_journey/"
        )
        if response.status_code == 200:
            appointment.refresh_from_db()
            ...
            print(
                f"❌ Step 5 failed: {response.status_code} - {response.content.decode()}"
            )
            return False

        # 6. Driver marks arrived (using API)
        response = api_client.post(
            f"/api/scheduling/appointments/{appointment.id}/arrive_at_location/"
        )
        if response.status_code == 200:
            appointment.refresh_from_db()
            ...
            print(
                f"❌ Step 6 failed: {response.status_code} - {response.content.decode()}"
            )
            return False

        # 7. Driver drops off (using API)
        response = api_client.post(
            f"/api/scheduling/appointments/{appointment.id}/drop_off_therapist/"
        )
        if response.status_code == 200:
            appointment.refresh_from_db()
            ...
            print(
                f"❌ Step 7 failed: {response.status_code} - {response.content.decode()}"
            )
            return False
        ...
        response = api_client.post(
            f"/api/scheduling/appointments/{appointment.id}/start_session/"
        )
        if response.status_code == 200:
            appointment.refresh_from_db()
            print(
                f"✅ Step 8: Therapist started session - status: {appointment.status}"
            )
        else:
            print(
                f"❌ Step 8 failed: {response.status_code} - {response.content.decode()}"
            )
            return False
