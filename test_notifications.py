...
def test_notifications():
    ...
    for role, role_display in CustomUser.ROLES:
        ...
        for notif in sample_notifications:
            print(
                f"    - {notif.user.username}: {notif.message[:50]}... (Read: {notif.is_read})"
            )
    ...
    for appt in recent_appointments:
        ...
        print(
            f"    - Therapist: {appt.therapist.username if appt.therapist else 'None'}"
        )
        ...
        for notif in notifications:
            print(
                f"      * {notif.user.username} ({notif.user.role}): {notif.message[:30]}..."
            )
        print()
    ...
    if operator and therapist and driver:
        print(
            f"  Found test users: {operator.username}, {therapist.username}, {driver.username}"
        )

        # Check if they have any notifications
        print("  Current notification counts:")
        print(
            f"    - {operator.username}: {Notification.objects.filter(user=operator).count()}"
        )
        print(
            f"    - {therapist.username}: {Notification.objects.filter(user=therapist).count()}"
        )
        print(
            f"    - {driver.username}: {Notification.objects.filter(user=driver).count()}"
        )
    else:
