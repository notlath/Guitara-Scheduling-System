...
def check_database_integrity():
    ...
    try:
        ...
        notifications_with_invalid_users = Notification.objects.filter(
            user__isnull=True
        )
        print(
            f"Notifications with null users: {notifications_with_invalid_users.count()}"
        )
        ...
        for notif in notifications:
            print(
                f"Notification {notif.id}: user={notif.user}, appointment={notif.appointment}"
            )
